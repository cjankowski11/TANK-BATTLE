...
SEND_PLAYER_NAME_AND_READY_STATUS = 1
RECIVE_PLAYERS = 0
RECIVE_GAME_STARTS = 1
            ...
            try:
                self.socket.sendto(
                    struct.pack(f"BB?{name_length}s",
                                SEND_PLAYER_NAME_AND_READY_STATUS, name_length,
                                self.ready, name), (self.host, self.port))
    ...
    def recive(self):
        while self.info.online and not self.info.game_running:
            try:
                ...
                if msg_type == RECIVE_GAME_STARTS:
